    ...
    for ann_file_name in sorted(os.listdir(img_dir)):
        ...
        if ext == '.txt':
            ...
            for line in file:  # each line is a keypoint pair to this picture
                ...
                if float(vWords[4])< (60*math.pi/180) or  float(vWords[4])> (120*math.pi/180):
                    if float(vWords[5])> 0.6 and float(vWords[6])> 0.6:
                        ...
                        obj['x1'] = float( obj['x0'] + 10.0*math.cos(obj['alpha']))
                        obj['y1'] = float( obj['y0'] + 10.0*math.sin(obj['alpha']))
                        """alpha = (float( vWords[3] ) - 0.1)/0.8*math.pi #alpha in rad
                        x1 = (float( vWords[1] ) + math.cos( alpha )*10)
                        y1 = (float( vWords[2] ) + math.sin( alpha )*10)
                        obj['x1'] = x1
                        obj['y1'] = y1"""
                        img['object'] += [obj]  #this is the keypoint pair of this image
        ...
        for instance_count in range( r_bound - l_bound ):
            ...
            for obj in all_objs:
                if obj['name'] in self.config['LABELS']:
                    ...
                    alpha = alpha/math.pi*0.8 + 0.1  # map into range 0.1...0.9
                    ...
                    if grid_x >= 0 and grid_y >= 0 and grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
                        ...
                        y_batch[instance_count, grid_y, grid_x, best_anchor, 3] = 1.  #confidence, is always 1.0 in gound truth
